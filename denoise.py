...
def denoised_task1(img, show=False):
    ...
    ret, denoised_lil = cv2.threshold(
        denoised_lil, 0, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)
...
def denoised_task2(img, show=False):
    ...
    denoised_lil = cv2.morphologyEx(img_grey, cv2.MORPH_CLOSE, kernel)
    ret, denoised_lil = cv2.threshold(
        denoised_lil, 230, 255, cv2.THRESH_BINARY_INV)
